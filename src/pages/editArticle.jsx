...
export default class Edit extends Component {
  ...
  getSelectText({code,arg}){


    let entry = this.refs.entryContent
    ...
    if(code === 'link') return editStore.setInputLink()
    entry.focus()
    ...
    // console.log(arg)
    //
    let {select} = this.getSelectionRange()
    //
    ...
    // if(code)
    let id = +new Date,defaultText = select.toString()?select.toString():'<br>'
    if(arg === 'pre' || arg === 'blockquote'){
      document.execCommand('insertHTML',false,'<'+arg+'><span id='+id+'>'+defaultText+'</span></'+arg+'><div><br></div>')
      ...
      document.execCommand(code,false,arg || null)
    ...
    // console.log({entry,html:entry.innerHTML})
    editStore.edit(entry.innerHTML)
   }
   checkNode(node){
     ...
     console.log({node})
      if(node.parentNode.localName === 'div'){
        ...
        if(node.localName === 'pre' || node.localName === 'blockquote') return 1
        else return 0
      }
      else return this.checkNode(node.parentNode)
   }
   keyEvent(e){
     let {select} = this.getSelectionRange()
     ...
     if ( e.keyCode === 13 && this.checkNode(select.anchorNode)) {
       console.log(this.checkNode(select.anchorNode))
      e.preventDefault()
              ...
              <div className={cs.toolsBar}>
                <div className={cs.barLeft}>
                  ...
                  {tools.map((item,i) => (
                    <div className={cs.toolbtnItem} key={i}>
                      {item.map((item2,i2) => (
                        <button className={cs.toolItem} onClick={() => this.getSelectText(item2)} key={i2}>
                ...
                <div className={cs.barRight}>
                  ...
                  <div className={cs.entryContent}>
                    ...
                    <div className={cs.entryEdit} contentEditable="true" onInput={(e) => editStore.edit(e.target.innerHTML)} onKeyDown={(e)=> this.keyEvent(e)} ref='entryContent'>
...
// getSelectText({code,path,len,defaultText}){
//
...
//   //
//   // // console.log(this.refs.entryContent.innerText)
...
//   entry.blur()
//   entry.focus()
//   let {select,range} = this.getSelectionRange(),title = range.toString()?range.toString():defaultText
//
//   let text = ''
//
//   switch (code) {
...
//
//   //
//   // document.execCommand('insertHTML',false,'<div><br></div><div">> <span id="ccc">1241241412</span></div><div><br></div>')
//   //
...
//   //
//   // var range2 = document.createRange();
//   //     range2.selectNode(cc);
//   //
//   //     console.log(range2)
//   // select.addRange(range2)
//   //
//   //
//   // var r2 = this.getSelectionRange()
//
//
//   // console.log(r2.select)
//   //
//   // r2.range.setStart(cc,2)
//
//   // r2.range.setEnd(r2.select.focusNode,8)
//
//
//   //
//   // document.execCommand('insertOrderedList',false,'321421')
//
//   // if(editStore.nextText === text) return
//
//   editStore.setNext(text)
//
//   let node = this.insert(range,document.createTextNode(text))
//
//
//
//   if(code != '----------') range.setStart(node,len)
//   else {
//     select.collapseToEnd()
//   }
//
//
//   if(code != '>' && code != '----------' && code != '-') range.setEnd(node,text.length-len)
//
//   entry.focus()
//
//
//   editStore.edit(entry.innerText)
//
//  }

// <Row type='flex' justify='center'>
//     <Col xl={{ span:12 }} lg={{ span: 12 }} md={{span:24}}  sm={{ span:24 }} xs={{span:24}} className={cs.entryContent}>
//       <span className={cs.entryPlaceholder+' '+isPlaceholder}>请输入正文</span>
//       <div className={cs.entryEdit} contentEditable="true" onInput={(e) => editStore.edit(e.target.innerText)} ref='entryContent'>
//
//       </div>
//     </Col>
//     <Col xl={{ span:12 }} lg={{ span: 12 }} md={{span:24}}  sm={{ span:24 }} xs={{span:24}} className={cs.outerContent}>
//       <div className={cs.outerShow} dangerouslySetInnerHTML={{__html:marked(editStore.articleText)}}>
//
//       </div>
//     </Col>
// </Row>
